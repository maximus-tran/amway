...
describe("Academy web_Search_Filter_LearningPath", () => {
  let driver;
  before(async () => {
  ...
  it("Redirect to the Search: View & Filter Result screen with the entered search terms activated", async () => {
    ...
    const screenshotDir = path.join(
      "auto-testing-report",
      "screenshots",
      "academyWebSearchFilterLearningPath",
    );
    await handleScreenShot(
      driver,
      screenshotDir,
      `verifySearchFilterLearningPath.png`,
    );
    addContext(this, {
      title: `Result screen is not existed in Favourite tab`,
      value: `./${screenshotDir.replace("auto-testing-report", "").replace(/\\/g, "/")}/verifySearchFilterLearningPath.png`,
    });
  });
  ...
  it("User should go to Courses tab by default with total Number Of Courses and it should show Learning path pill tab next to Courses pill tab with the and total Number Of Learning Paths", async () => {
    ...
    // Click Learning Path pill tab
    const pillTab = await driver.findElements(By.className("tab_name___ll3L "));
    await pillTab[1].click();
    ...
    await handleScreenShot(
      driver,
      screenshotDir,
      `totalNumberOfLearningPath.png`,
    );
    addContext(this, {
      title: `Total Number Of LearningPath and it should show Learning path pill tab next to LearningPath pill tab`,
      value: `./${screenshotDir.replace("auto-testing-report", "").replace(/\\/g, "/")}/totalNumberOfLearningPath.png`,
    });
  })
  ...
  it("It should display the search result that matches between the Courses/ Learning Path title, Description, Author/Speaker Name or CST and key words", async () => {
    ...
    if (haveResult) {
      ...
      for (let item = 0; item < loopCount; item++) {
        ...
        // Check New, Required, Recommended tags displayed at thumbnail
        const isDisplayedTags = await checkElementExists(
          driver,
          "className",
          "pill_container__91xC1"
        )
        assert.ok(isDisplayedTags, "New / Required / Recommended tags is not displayed at thumbnail")
