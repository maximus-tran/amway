...
describe("Verify learning path detail", () => {
  ...
  it("Verify that can launch the Learning Path successfully as per selected language", async () => {
    ...
    for (let item = 0; item < loopCount; item++) {
      //Select a course
      await learningPathsList[1].click();
      ...
      assert.ok(isDisplayedDropDown, "Dropdown is not displayed")
      if (isDisplayedDropDown) {
        ...
        const subTitleNew = await driver.wait(until.elementLocated(By.className("card_name__HOcFA")), timeOut).getText();
        assert.ok(isLanguageMatching(subTitleNew, validLanguage), `Title (${subTitleNew}) not matching to English`);
        const subDescriptionNew = await driver
          .wait(until.elementLocated(By.className("card_description__Ax4dg ellipses")), timeOut)
          .getText();
